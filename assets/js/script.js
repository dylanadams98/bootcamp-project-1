...
async function DisplayStockData(stocksTicker, from, to) {
  ...
  try {
    ...
    errorMsg.textContent = `Error fetching stock graph: ${error}`;
...
async function DisplayNews(stocksTicker, from, to) {
  ...
  try {
    ...
    for (var i = 0; i < newsData.length; i++) {
      ...
      newsURL.textContent = "Visit Page";
...
async function DisplayInfo(stocksTicker) {
  try {
    const infoData = await getNews(stocksTicker);

    for (var i = 0; i < infoData.length; i++) {

      var infoBox = document.createElement("");
      infoBox.setAttribute("class", "info-box");

      var infoLogo = document.createElement("img");
      infoLogo.setAttribute("img", infoData[i].logo_url);

      var infoName = document.createElement("p");
      infoName.textContent = `${infoData[i].name}`;

      var infoDesc = document.createElement("a");
      infoDesc.textContent = `${infoData[i].description}`;


      myChart.appendChild(infoLogo);
      myChart.appendChild(infoName);
      myChart.appendChild(infoDesc);

    }
    infoSuccess = true;
  } catch (error) {
    var errorMsg = document.createElement("h1");
    errorMsg.textContent = `Error fetching info : ${error}`;
    chartContainer.appendChild(errorMsg);
    infoSuccess = false;
  }
}